...
USER_AGENT = "linux-historic-mirror/1.0 (+archival backup)"
...
def safe_output_path(root: Path, rel_posix: str) -> Path:
    # Decode URL path and make sure it remains below root.
    rel = PurePosixPath(urllib.parse.unquote(rel_posix))
    if rel.is_absolute() or ".." in rel.parts:
        raise ValueError(f"Unsafe archive path: {rel_posix!r}")

    out = root.joinpath(*rel.parts).resolve()
...
path = Path("mirror_linux_historic.py")
path.write_text(script, encoding="utf-8")
print(f"Script saved to: {path.absolute()}")
